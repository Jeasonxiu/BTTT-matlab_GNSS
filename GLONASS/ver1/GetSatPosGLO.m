function [sat_pos,sat_vel,sat_LS] = GetSatPosGLO(eph_glo,icol,tc,deltat)
%
% <input>
...
WGS_xyz = PZ2WGS_JSG(x(3:5)); % 좌표변환 - 좌표변환을 어디 부분에 적용할지는 고민/ 1)좌표계산 후 좌표변환 2) 좌표변환 후 좌표계산
x(3:5) = WGS_xyz;
%%
dif = tc - gs; % 궤도력과 epoch와의 시간차
if dif < 0 % backward
    deltat = -deltat;
    it = fix(dif/deltat);
    hat = rem(dif,deltat);
