...
DOY = '153'; YY = '16';         % Day of Year    
TIME = 'h';                     % Logging Hour(9a,10b,11c,12d,13e,14f,15g,16h,17i,18j,19k,
                                %             20l,21m,22n,23o,24p,1q,2r,3s,4t,5u,6v,7w,8x)  
DEVICE = 'tab';

NMEAfile = 'DRUA1532.ubx';
% NMEAfile = 'jprA034x_s6.txt';
NMEAQM = writeNMEA4(NMEAfile);
...
%                 cp = CCC * (Tgd) + dIono + dTrop_G + prc;   
                H = [ -vec_rho(1)/rho -vec_rho(2)/rho -vec_rho(3)/rho 1];
                HTH = HTH + H'*W*H;
                HTy = HTy + H'*W*y;
                HT_prc = HT_prc + H'*0.5*cp;                                            % Correction-Projection
            end
