...
nEst = 0;
x_prc = zeros(4,1);
PRCPRN = zeros(NoEpochs,length(unique(PRC_Sorted(:,2)))+1);
NMEAPRN = zeros(NoEpochs,length(unique(NMEAQM(:,5)))+1);
for j = 1:NoEpochs
    for Iter = 1:MaxIter
        HTH = zeros(4,4);
        HTy = zeros(4,1);
       ...
       prcprn = sort(PRC_1);
        for jjj = 1:length(prcprn(:,2))
            PRCPRN (j,1) = FinalTTs(j);
            PRCPRN (j,jjj+1) = prcprn(jjj,2);
        end
        for kkk = 1:length(NMEAQM_1(:,5))
            NMEAPRC (j,1) = FinalTTs(j);
            NMEAPRC (j,kkk+1) = NMEAQM_1(kkk,5);
        end
       %% NMEA 에 PRC prn이 없는 경우를 대비하기 위해서
        existprn = intersect(unique(NMEAQM_1(:,5)), PRC_1(:,2));
        arrSV = zeros(length(existprn),1);
        for k = 1:length(existprn)
            arrSV(k) = find(NMEAQM_1(:,5) == existprn(k,1));
        end
        NMEAQM_1 = NMEAQM_1(sort(arrSV),:);                 % PRC 내의 prn으로 sorting
