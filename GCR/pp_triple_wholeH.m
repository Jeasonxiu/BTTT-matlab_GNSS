...
% ephGLO = eph;
% ephGLO=ephGLO(ephGLO(:,18)<400&ephGLO(:,18)>300,:);ephGLO(:,1)=ephGLO(:,1);
%
...
% nEst = 0;
% % load('pp_triple_wholeH.mat');
load('pp_triple_170407d.mat');
% load('pp_triple_170407m.mat');
% load('pp_triple_170616b.mat');
...
for j = 1:30:NoEpochs
    ...
    for iter = 1:Maxiter
        ...
        vec_site = x(1:3)';
        ZHD = TropGPTh(x(1:3), gw, gs); %: TROP: GPT
        for i = 1:NoSats
...
%                 dIono = klobuchar(al, be, gs, az, el, x(1:3)); % 이온층 보정(Klobuchar 모델)
                dTrop = ZHD2SHD(gw, gs, x(1:3), el, ZHD); % 대류권 보정
...
%                 if el >=10 % 임계고도각
                ...
                if iter == 1
                    UsedSats(cnt3,1) = gs;
                    if select == 1
                        UsedSats(cnt3,prn-100) = prn-100;
                    elseif select == 3
                        UsedSats(cnt3,prn-200) = prn-200;
                    end
