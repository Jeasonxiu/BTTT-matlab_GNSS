...
%% 자료처리 원시데이터
% QMfile = 'QM170125_A'; navfile = 'brdm0250.17p'; YY = 17; DOY =025;
% QMfile = 'QM170407m'; navfile = 'brdm0970.17p'; YY = 17; DOY =097;
% QMfile = 'QM170616b'; navfile = 'brdm1670.17p'; YY = 17; DOY =167;
QMfile = 'QM170616b'; navfile = 'brdm0970.17p'; YY = 17; DOY =166;
%% PRC
...
% load('DGPSCP_triple_170407m.mat');
% load('DGPSCP_triple_170616b.mat');
...
FinalTTs = intersect(FinalTTs,nmea(:,1));
NoEpochs = length(FinalTTs);
tic;
for j = 1:NoEpochs
    ...
    NMEA_1e = nmea(find(nmea(:,1) == gs),:);
    if NMEA_1e(1,4) == 0
        NMEA_xyz = gd2xyz([NMEA_1e(1,2), NMEA_1e(1,3), NMEA_1e(1,4)]);
    else
        NMEA_xyz = gd2xyz([NMEA_1e(1,2), NMEA_1e(1,3), NMEA_1e(1,4)]); end
    x(1:3) = NMEA_xyz';
    existprn = intersect(QM1e(:,2), unique(eph(find(eph(:,22) == 0),18)));
    existprn = intersect(existprn, PRC_1e(:,2));
    existprn = intersect(existprn, NMEA_1e(:,5));
...
%             if el >=eleCut % 임계고도각
                ...
                if prn > 100 && prn < 200
                    ...
                    cp = - dTrop - dIono - PRC; % gps 계산값
%                     cp = - dTrop - dIono; % gps 계산값
...
%             end
        ...
        if NoSatsUsed >= 5
            ...
            HTy_CP = H(1:cnt2-1,:)'*inv(W(1:cnt2-1,1:cnt2-1))*CP(1:cnt2-1,:);   % CP
            xhat = inv(HTH) * HTy;
            xhat_CP = -inv(HTH) * HTy_CP;    % CP
%             x = x + xhat;
            x = x + xhat_CP;
            if iter == Maxiter;
                nEst = nEst + 1;
