...
% for j = 1:804
    ...
    visiSat(j,1) = gs; visiSat(j,2) = NoSats; visiSat(j,3) = GloSats; 
...
%                         W = MakeW_elpr(el);
                        ...
                        com_d = DistNorm_d + x(4) - CCC*dtSat_d -prc;          % DGLO
                        
                        y = obs - com;                                          % GLO PP
                        GLOo_c(j,prn) = y;
                        y_d = obs - com_d;                                      % DGLO
                        H = [-DistXYZ(1)/DistNorm -DistXYZ(2)/DistNorm -DistXYZ(3)/DistNorm 1];               % GLO PP
                        H_d = [-DistXYZ_d(1)/DistNorm_d -DistXYZ_d(2)/DistNorm_d -DistXYZ_d(3)/DistNorm_d 1]; % DGLO
                        HTH = HTH + H'*W*H;                     % GLO PP
                        HTH_d = HTH_d + H_d'*W*H_d;             % DGLO
                        HTy = HTy + H'*W*y;                     % GLO PP
                        HTy_d = HTy_d + H_d'*W*y_d;             % DGLO
%                     end
                end
            end
        end
...
estm = estm(1:nEst,:);
estm_d = estm_d(1:nEst,:);
%% 측위오차 분석 & 그래프 작성
% [dXYZ, dNEV] = PosTErrors4(estm(:,1), TruePos, estm(:,2:5),visiSat);
% [dXYZ_d, dNEV_d] = PosTErrors4(estm_d(:,1), TruePos, estm_d(:,2:5),visiSat);

% A2
% estm = [estm(1:153, :);estm(159:474, :);estm(480:488, :);estm(498:503, :);estm(513:520, :)];
% estm_d = [estm_d(1:153, :);estm_d(159:474, :);estm_d(480:488, :);estm_d(498:503, :);estm_d(513:520, :)];
% B2
estm = [estm(1:227, :);estm(234:235, :);estm(249:560, :)];
estm_d = [estm_d(1:227, :);estm_d(234:235, :);estm_d(249:560, :)];
[dXYZ, dNEV] = PosTErrors4(estm(:,1), TruePos, estm(:,2:5),visiSat);
[dXYZ_d, dNEV_d] = PosTErrors4(estm_d(:,1), TruePos, estm_d(:,2:5),visiSat);

save(strcat('a_GPS_DGPS_',FileQM(5),FileQM(13),'.mat'), 'estm', 'estm_d', 'TruePos', 'visiSat');
