...
% LeapSec = 18;           % GLO
% %% EPH 생성
% % FileNav = strcat('brdm', num2str(DOY,'%03d'), '0.', num2str(year-2000,'%02d'), 'p');   %: Navigation RINEX file
% FileNav = 'brdm2570.17p';   %: Navigation RINEX file
    ...
    for iter = 1:Maxiter
        ...
        for j=1:length(ExistPRN)
            ...
            if prn < 300
                ...
                %             STT=0;
                STT = GetSTTbrdm(gs, eph, icol, EstPos(1:3)); % 신호전달시간 계산
            ...
            elseif prn > 300
                ...
                if icol == icolArr_before(icolArr_before(:,1)==prn,2) % icol 변화 없으면 이전 epoch으로 계산
                    %                 disp(1)
                    STT = GetSTTbrdc_GLO_ver3(gs, SatPosArr_before, jcol, EstPos(1:3), deltat); % 이전 epoch값으로 신호전달시간 계산
                    tc = gs - STT;
                    tc = tc - LeapSec - TauC;
                    ...
                    %                 disp(2)
                    STT = GetSTTbrdc_GLO(gs, ephGLO, icol, EstPos(1:3), deltat); % 방송궤도력으로 신호전달시간 계산
                    tc = gs - STT;
                    tc = tc - LeapSec - TauC;
            ...
            vec_rho_cp_alm = SatPos - EstPos(1:3);
