...
QMRv = load('QDSBJ_18107m');    % 18107 JAVAD-B
...
Q = MakeQkt(NoSats);
P = eye(3+NoSats);                            % xyz + 기준위성을 제외한 NoSats
P(1:3,1:3) = P(1:3,1:3) * 2;                    % P xyz 초기값 선언
P(4:3+NoSats,4:3+NoSats) = P(4:3+NoSats,4:3+NoSats) * 10;       % P 위성 별 N 초기값 선언
A = eye(3+NoSats);                            % xyz + 기준위성을 제외한 NoSats
R = eye(NoSats,NoSats);                     % 기준위성과 나머지 위성 조합수
%% Rover 초기좌표
...
for i = 1:length(FinalTTs)
    ...
    %% 모호정수 초기값 생성
    if N(1) == 0
        N = N_init(Obs_Bs, Obs_Rv);             % 첫 Epoch 에서 N 초기값 생성
    else
        N = x(4:end);
    end
    %% R: measurement noise
...
figure(33)
subplot(NoSats,1,NoSats)
xlabel('Hours')
for i = 1:NoSats
    prn = SatsList(i);
    if prn == prn_RS
        subplot(NoSats,1,i)
        plot(P_tHour, P_whole(:,4+i),'r.:'); hold on; grid on;
