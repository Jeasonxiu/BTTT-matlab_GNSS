...
% clc;clear;eph_file = 'brdm2750.15p';
%% 파일 핸들과 결과 저장 행렬 초기화
...
while ready == 0
    ...
    if length(s) > 7                    %[G,E,C,J]  %[R,S]
        ...
        if SSI == 'G'|| SSI == 'C'|| SSI == 'R'|| SSI == 'E'|| SSI == 'J'|| SSI == 'S' % IRNSS 패스해주려고 임시로 만든 부분 
            
        %% 여기서부터 다시 (IRNSS 상관없는) 원래 있던부분   
        ...
        else            % IRNSS 패스해주려고 임시로 만든 부분    
        end             % IRNSS 패스해주려고 임시로 만든 부분
    else        
        ready=1;
    end
