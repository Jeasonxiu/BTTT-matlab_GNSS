function [W] = DD_cofactor_matrix(elBs, elRv, SNRBs, SNRRv, W_index);

% SYNTAX:
%   [W] = DD_cofactor_matrix(elBs, elRv, SNRBs, SNRRv, W_index);
%
% INPUT:
...
%   elBsRs = RS satellite elevations (Base)
%   elRvRs = RS satellite elevations (Rover)
%   SNRBs = signal-to-noise ratio (Base)
%   SNRRv = signal-to-noise ratio (Rover)
...
if (W_index == 0)
    ...
    W_Bs_RS = WBs(1,1); % ROVER-PIVOT
    W_Rv_RS = WRv(1,1); % MASTER-PIVOT
    WBs(1) = [];
    WRv(1) = [];
    W_Bs_OS = WBs;                % ROVER-generic satellite (without pivot)
    W_Rv_OS = WRv;                % MASTER-generic satellite (without pivot)

    %code-code or phase-phase co-factor matrix Q construction
    W = (W_Bs_RS + W_Rv_RS) * ones(length(elBs)-1) + diag(W_Bs_OS + W_Rv_OS);
end
