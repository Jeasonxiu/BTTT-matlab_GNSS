...
sec = UTC(:,6);
% Validate vectors
validateattributes(year,{'numeric'},{'>=',1});
...
p = inputParser;
addRequired(p,'reduction',@ischar);
...
addParamValue(p,'dNutation',zeros(len,2),@(x) validateattributes(x,{'numeric'},...
    {'real','finite','nonnan','size',[len,2]}));
addParamValue(p,'dCIP',zeros(len,2),@(x) validateattributes(x,{'numeric'},...
    {'real','finite','nonnan','size',[len,2]}));
