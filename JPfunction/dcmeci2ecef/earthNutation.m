...
%                   JPL. It can be: '405', '421' or '423'. If it is not
%                   defined, the default model is '405'.
...
switch nargin
    ...
    case 2
        ...
        de = lower(de);
        de = validatestring(de,validDatabases);
        errorFlag = 'error';
    case 3
        de = varargin{1};
        errorFlag = lower(varargin{2});
        % Validate database
        if isnumeric(de)
            de = num2str(de);
        end
        ...
        de = validatestring(de,validDatabases);
        % Validate error flag
        errorFlag = validatestring(errorFlag,validError);
end
