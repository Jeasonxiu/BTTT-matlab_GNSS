function [DDdXYZ, DDdNEV, DDdis, DDrms, DDstd] = PostErrorsDDmv2(estm, Base, Truedis, ymin, ymax, visiSat); 
%
...
%           S1sum: sum of base prn's snr, sum of rover prn's snr, type of
%           snr status, num of base's sats, num of rover's sats,
%
...
% xlim([0 length(estm)])
xlim([min(tHour) max(tHour)])
stairs(tHour, visiSat(:,2),'k.:');
stairs(tHour, visiSat(:,7),'r.:');
