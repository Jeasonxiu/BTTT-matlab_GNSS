...
% 베이스와 로버의 dTrop과 dIono를 계산하여 O-C에 적용하는 코드
...
% Bs = PP_gc_v1(QMfileBs,eph,TruePos,DOY,YY,0);              % with Correction and Weighting
[Bs, dTdI_Bs] = PP_gc_v2(QMfileBs,eph,TruePos,DOY,YY,0);              % with Correction and Weighting
[Rv, dTdI_Rv] = PP_gc_v2(QMfileRv,eph,TruePos,DOY,YY,0);              % with Correction and Weighting
...
for j = 1:NoEpochs
    ...
    QM_Bs_dTdI_1e = dTdI_Bs(find(dTdI_Bs(:,1) == gs),:);                    % Base dTrop, dIono
    ...
    QM_Rv_dTdI_1e = dTdI_Rv(find(dTdI_Rv(:,1) == gs),:);                    % Base dTrop, dIono
    
    PP_sats = intersect(QM_Bs_dTdI_1e(:,2), QM_Rv_dTdI_1e(:,2));
    Sats = intersect(QM_Bs_1e(:, 2), QM_Rv_1e(:, 2));
    Sats = intersect(Sats, unique(eph(find(eph(:,22) == 0),18)));
    Sats = intersect(Sats, PP_sats);
    NoSats = length(Sats);
    ...
    if NoSats >= 6
        ...
        %% 기준위성 dTrop, dIono 추출
        Bs_dTdI_RS = QM_Bs_dTdI_1e(find(QM_Bs_dTdI_1e(:,2) == RS),:);
        Rv_dTdI_RS = QM_Rv_dTdI_1e(find(QM_Rv_dTdI_1e(:,2) == RS),:);
            ...
            switch SYS
                ...
                case 2
                    ...
                    for kS = 1:length(BDS(:,1))
                        ...
                        if el >= eleCut
                            ...
                            w = DDMakeW_elsnr(elBsOS, elRvOS, BDS_snr_Bs_OS, BDS_snr_Rv_OS, 4);
                            W(cnt2,cnt2) = w;
                            %% H 행렬 계산 파트
                            %             H(1,1) = vec_RvRS(1)/com_RvRS - vec_RvOS(1)/com_RvOS;
                            %             H(1,2) = vec_RvRS(2)/com_RvRS - vec_RvOS(2)/com_RvOS;
                ...
                case 4
                    for kS = 1:NoSats
                        ...
                        OS = Sats(kS,1);
                      %% 다른위성 dTrop, dIono 추출
                        Bs_dTdI_OS = QM_Bs_dTdI_1e(find(QM_Bs_dTdI_1e(:,2) == OS),:);
                        Rv_dTdI_OS = QM_Rv_dTdI_1e(find(QM_Rv_dTdI_1e(:,2) == OS),:);
...
%                         vec_OS = RotSatPos(vec_OS, STT);
                        ...
                        com = (com_BsRS - com_RvRS) - (com_BsOS - com_RvOS);
%                         com = com + (Bs_dTdI_RS(3) - Rv_dTdI_RS(3)) - (Bs_dTdI_OS(3) - Rv_dTdI_OS(3));
%                         com = com + (Bs_dTdI_RS(4) - Rv_dTdI_RS(4)) - (Bs_dTdI_OS(4) - Rv_dTdI_OS(4));
                        y = obs - com;
                        Y(cnt2,1) = y;
...
[DDdXYZ, DDdNEV, DDdis, DDrms, DDstd] = PostErrorsDDmv3(estm, Base, Truedis,8, 12, SYS);    % 임의의 장소에서 이동 측위시
