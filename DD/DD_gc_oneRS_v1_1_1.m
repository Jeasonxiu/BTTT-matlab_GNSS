% 이중차분 시 베이스와 로버 별로 위성의 위치를 따로 계산해서 처리하는 코드
    ...
    if NoSats >= 6
        ...
        for Iter = 1:MaxIter
           ...
           %% 전체 기준위성 좌표 먼저 계산 - Bs 기준
            ...
            %% 전체 기준위성 좌표 먼저 계산 - Rv 기준
            STT_rv = GetSTTbrdm(gs, eph, icol, x(1:3));                     % 신호전달시간 계산
            tc_rv = gs - STT_rv;
            vec_RS_rv = GetSatPosNC_GC(eph, icol, tc_rv);
            vec_RS_rv = RotSatPos(vec_RS_rv, STT_rv);                              % 지구 자전효과 고려
            ...
            %% GPS 기준위성 좌표 먼저 계산 - Rv 기준
            STT_rv = GetSTTbrdm(gs, eph, icol, x(1:3));                     % 신호전달시간 계산
            tc_rv = gs - STT_rv;
            vec_GPSRS_rv = GetSatPosNC_GC(eph, icol, tc_rv);
            ...
            STT_rv = GetSTTbrdm(gs, eph, icol, x(1:3));                     % 신호전달시간 계산
            tc_rv = gs - STT_rv;
            vec_BDSRS_rv = GetSatPosNC_GC(eph, icol, tc_rv);
            vec_BDSRS_rv = RotSatPos(vec_BDSRS_rv, STT_rv);                              % 지구 자전효과 고려
            ...
            switch SYS
                case 1
                    ...
                    for kS = 1:length(GPS(:,1))
                        ...
                        STT_rv = GetSTTbrdm(gs, eph, icol, x(1:3)); % :  OS 위성위치를 Rover 기준으로 변경 4/7/117
                        tc_rv = gs - STT_rv;
                        vec_GPSOS_rv = GetSatPosNC_GC(eph, icol, tc_rv);
                        vec_GPSOS_rv = RotSatPos(vec_GPSOS_rv, STT_rv);
                        ...
                        vec_RvOS = vec_GPSOS_rv - x(1:3);    com_RvOS = norm(vec_RvOS);
                        ...
                        if el >= eleCut
                            ...
                            w = DDMakeW_elsnr(elBsOS, elRvOS, GPS_snr_Bs_OS, GPS_snr_Rv_OS, 0);
                            W(cnt2,cnt2) = w;
                ...
                case 2
                    ...
                    for kS = 1:length(BDS(:,1))
                        ...
                        STT_rv = GetSTTbrdm(gs, eph, icol, x(1:3)); % :  OS 위성위치를 추정좌표 기준으로 변경 11/9/14
                        tc_rv = gs - STT_rv;
                        vec_BDSOS_rv = GetSatPosNC_GC(eph, icol, tc_rv);
                        vec_BDSOS_rv = RotSatPos(vec_BDSOS_rv, STT);
                        %% DD 계산치 생성 파트 - 거리계산치를 각각 계산한 다음 DD 계산치 계산
                        vec_BsRS = vec_BDSRS - TruePosBs_PP';  com_BsRS = norm(vec_BsRS);
                        vec_RvRS = vec_BDSRS_rv - x(1:3);    com_RvRS = norm(vec_RvRS);
                        vec_BsOS = vec_BDSOS - TruePosBs_PP';  com_BsOS = norm(vec_BsOS);
                        vec_RvOS = vec_BDSOS_rv - x(1:3);    com_RvOS = norm(vec_RvOS);
                        com = (com_BsRS - com_RvRS) - (com_BsOS - com_RvOS);
                        y = obs -com;
                        Y(cnt2,1) = y;
                        if Iter == 1
...
%                             w = 1;
                            ...
                            w = DDMakeW_elsnr(elBsOS, elRvOS, BDS_snr_Bs_OS, BDS_snr_Rv_OS, 0);
                            W(cnt2,cnt2) = w;
                            %% H 행렬 계산 파트
...
%                         OBS2(cnt,1:2) = [(L1_Bs_RS - L1_Rv_RS) (L1_Bs_OS - L1_Rv_OS)];
                        ...
                        STT_rv = GetSTTbrdm(gs, eph, icol, x(1:3)); % :  OS 위성위치를 추정좌표 기준으로 변경 11/9/14
                        tc_rv = gs - STT_rv;
                        vec_OS_rv = GetSatPosNC_GC(eph, icol, tc_rv);
                        vec_OS_rv = RotSatPos(vec_OS_rv, STT);
                        %% DD 계산치 생성 파트 - 거리계산치를 각각 계산한 다음 DD 계산치 계산
                        vec_BsRS = vec_RS - TruePosBs_PP';  com_BsRS = norm(vec_BsRS);
                        vec_RvRS = vec_RS_rv - x(1:3);    com_RvRS = norm(vec_RvRS);
                        vec_BsOS = vec_OS - TruePosBs_PP';  com_BsOS = norm(vec_BsOS);
                        vec_RvOS = vec_OS_rv - x(1:3);    com_RvOS = norm(vec_RvOS);
                        com = (com_BsRS - com_RvRS) - (com_BsOS - com_RvOS);
                        y = obs - com;
                        Y(cnt2,1) = y;
...
%                             w = DDMakeW_snrDiff([snr_Bs_RS, snr_Rv_RS],[snr_Bs_OS, snr_Rv_OS]);
                            w = DDMakeW_elsnr(elBsOS, elRvOS, snr_Bs_OS, snr_Rv_OS, 3);
                            W(cnt2,cnt2) = w;
                            % H 행렬 계산 파트
                            %                 H(1,1) = vec_RvRS(1)/com_RvRS - vec_RvOS(1)/com_RvOS;
                            %                 H(1,2) = vec_RvRS(2)/com_RvRS - vec_RvOS(2)/com_RvOS;
