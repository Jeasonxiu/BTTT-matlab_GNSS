function [SatsEl, highELprn] = PickRSelGPSGLO(gs, LeapSec, Sats, ephgps, ephglo, Sat_ar, TauC, vec_site)
%
...
for kS = 1:NoSats;
    prn = Sats(kS);
    if prn < 100
        icol = PickEPH(ephgps, prn, gs);
        vec_sat = GetSatPosNC(ephgps, icol, gs);
        vec_rho = vec_sat - vec_site;
        [az, el] = xyz2azel(vec_rho, AppLat, AppLon);
    else
        tc = gs - LeapSec;
        icol=PickEPH_GLO2(ephglo, prn-100, tc);
        
        TauN=ephglo(icol,12); GammaN=ephglo(icol,13); %: tau & gamma 시계오차 보정에 사용
        ch_num=ephglo(icol,16); %: channel number 전리층 보정에 사용
        
        % 신호전달시간 계산
        STT = GetSTTbrdcGLO2(Sat_ar,gs,prn-100,vec_site');
        % LeapSecond & 신호전달 시간을 보정한 위성 위치 산출
        [vec_sat, SatVel] = SatPosLS_STT(Sat_ar,gs,prn-100,LeapSec,STT,TauC);
        vec_rho = vec_sat - vec_site;
