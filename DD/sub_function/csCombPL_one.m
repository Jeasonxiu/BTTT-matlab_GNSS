...
%  7/13/15-- QM 파일 대신 QM 행렬을 읽어서 작업하도록 수정
...
colPH = 4;
%% 관측치 세트 읽음
% [arrQM, dummy, dummy] = ReadQM(qm_file); %: 7/13/15 수정(comment-out)
...
tS = tHour(1); tE = tHour(end);
yObs = comb(:,3);
plot(tHour, y Obs, 'o:'); 
% axis([tS tE min(yObs) max(yObs)]);
% axis([tS tE -20 20]);
% xlabel('Hour'), ylabel('Code-Phase Comb (m)'), title(num2str(prn))
xlabel('Hour'), ylabel('Code-Phase Comb (n)'), title(num2str(prn))
