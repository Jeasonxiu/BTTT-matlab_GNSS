...
FileQM_Bs ='QM170125_A';   %  GSDA (가산동 옥상 A)
FileQM_Rv ='QM170125_B';   %  GSDB (가산동 옥상 B)
...
%     Sats_for_Process = unique(ObsType_Selected_QM_Bs(:,2));
...
for i = 1:NoEpochs
    ...
    sub_QM_Bs = ObsType_Selected_QM_Bs(IndexS_QM_Bs,:);
    sub_QM_Rv = ObsType_Selected_QM_Rv(IndexS_QM_Rv,:);
    ...
    [SatsAZEl, indxRS]=PickRSel(gs,Sats_for_Process, eph, rcvPos);      % RS : Reference Satellite 기준위성
    ...
    %% 반복 계산 시작
        ...
        tc = gs - STT;
        vec_Sat_RS = GetSatPos(eph, indxEPH_RS, tc);      % 해당 위성의 위치 산출
        ...
        for j = 1:NoSats
            ...
            tc = gs - STT;
            vec_Sat_OS = GetSatPos(eph, indxEPH_OS, tc);       % 해당 위성의 위치 산출
