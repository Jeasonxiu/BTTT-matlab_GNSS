...
% FileQM='test4_1_L';
    ...
    arrQM = arrQM(find(arrQM(:,1) < 113171 | arrQM(:,1) > 113173),:); 
    arrQM = arrQM(find(arrQM(:,1) < 113221 | arrQM(:,1) > 113222),:); 
    arrQM = arrQM(find(arrQM(:,1) < 113313 | arrQM(:,1) > 113318),:); 
    arrQM = arrQM(find(arrQM(:,1) < 113323 | arrQM(:,1) > 113330),:); 
    arrQM = arrQM(find(arrQM(:,1) < 113571 | arrQM(:,1) > 113579),:);
    arrQM = arrQM(find(arrQM(:,1) < 113629 | arrQM(:,1) > 113640),:);
    % break
    yyS = num2str(YY,'%02d');
    ...
    for j = 1:NoEpochs % 60:65%
        ...
        for iter = 1:Maxiter
            ...
            for i = 1:NoSats
                ...
                %             STT=0;
                STT = GetSTTbrdc_(gs, eph, icol, x(1:3)); % 신호전달시간 계산
                if prn > 100 && prn < 200
                    tc = gs - STT ;             % 신호전달시간 보정
