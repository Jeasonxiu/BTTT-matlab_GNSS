% function [estm] = DPP_g_SP(QMfile,eph,TruePos, RTCM, DOY, YY)
...
QM_prrSigmaMps = SelectQM_gc(QM_prr, g_ObsType_prrSigmaMps, c_ObsType_snr);
...
for j = 1:NoEpochs
    ...
    QM1e_prrSigmaMps = QM_prrSigmaMps(indexQM,:);
    ...
    for iter = 1:Maxiter
        ...
        for i = 1:NoSats
            ...
            if el >=eleCut % 임계고도각
                W = 1;
                W = 1/QM1e_PrSigmaM(find(QM1e_PrSigmaM(:,2) == prn),4);
                if prn > 100 && prn < 200
