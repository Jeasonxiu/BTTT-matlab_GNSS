function [estm] = PP_grc(QMfile,eph,TruePos, DOY, YY, W_index)

%
%   function [estm] = PP_grc(QMfile,eph,TruePos, DOY, YY)
%
%   Read the files(QMfile, eph, TruePos, DOY, YY) and estimate Position(GPS/BDS/GLO) with weighting(goGPS)
% 
%
%   input QMfile
...
for j = 1:NoEpochs % 60:65%
    ...
    for iter = 1:Maxiter
        ...
        if NoGPSsUsed+NoBDSsUsed+NoGLOsUsed >= 6
            P = inv(HTH);
            xhat = inv(HTH) * HTy;
            x = x + xhat;
