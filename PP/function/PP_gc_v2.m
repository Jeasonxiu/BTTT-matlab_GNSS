function [estm, dTdI] = PP_gc_v2(QMfile,eph,TruePos, DOY, YY, W_index)

%
%   function [estm] = PP_gc_v2(QMfile,eph,TruePos, DOY, YY)
%
%   Read the files(QMfile, eph, TruePos, DOY, YY) and estimate
%   Position(GPS/BDS) with weighting(goGPS) and dTrop/dIono per epoch
%
%   input QMfile
...
%
%   coded by Joonseong Gim, Mar 17, 2017
%
%
...
for j = 1:NoEpochs
    ...
    for iter = 1:Maxiter
        ...
        for i = 1:NoSats
            ...
            if el >=eleCut % 임계고도각
                %                 W = 1;
                W = MakeW_elsnr_v1(el,snr,W_index);
                if prn > 100 && prn < 200
                    PRC = 0;
                ...
                if iter == 1
                    dTdI(cnt,1:4) = [gs prn, dTrop, dIono];
                    cnt=cnt+1;
                end
